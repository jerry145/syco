#!/usr/bin/python2
# coding=utf-8
# code by Saqii D'Lewixx
# my facebook (https://www.facebook.com/baap.hu.tera.id.ura.ky.dekha )

#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By Saqii D'Lewixx.
...
# LO KONTOL
 \033[0;96m  @@@****Saqii****@@@
 \033[0;96m/  ___>/  _  \/  _  \/___\/___\® \033[0m|| Created By Saqii D'Lewixx
 \033[0;96m|___  ||  _  ||  |  ||   ||   |  \033[0m|| Github.com/Saqii D'Lewixx
 \033[0;96m <_____/\__|__/\___\ \\___/\___/\033[0;91mv2.0  \033[0m|| Facebook.com/baap.hu.tera.id.ura.ky.dekha'''
  \033[0;96m                    /          
...
def Saqii D'Lewixx ():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);Saqii D'Lewixx()
...
def Saqii D'Lewixx():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');Saqii D'Lewixx()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');Saqii D'Lewixx()
    ...
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] jangan kosong kentod!'%(N,M,N);time.sleep(2);Saqii D'Lewixx()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N)); Saqii D'Lewixx ()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N)); Saqii D'Lewixx ()
'''
...
def info_tools():
    ...
    print '\n %s[%s>%s] Yt       : Saqii Tech.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Author   : Saqii D'Lewixx. '%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/ Saqii D'Lewixx '%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Facebook :https://www.facebook.com/baap.hu.tera.id.ura.ky.dekha'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Fanspage : https://www.facebook.com/107420165014512/posts/128930152863513/?app=fbl%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/Saqii_tech'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Website  : https://www.Saqii tech.my.id'%(N,H,N);time.sleep(0.07)
                            ...
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        ...
                                        __Saqii__.submit(self.__api__, kimochi, ysc)
                                    except: pass
